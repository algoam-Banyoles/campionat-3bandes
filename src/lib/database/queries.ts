...
export interface RankingWithPlayers {
	...
	posicio: number;
	soci_id: number;
	mitjana_referencia: number | null;
	data_entrada: string;
...
export class OptimizedQueries {
	...
	async getRankingWithPlayers(includeActiveChallenges = false): Promise<RankingWithPlayers[]> {
		...
		try {
			...
			let rankingData: RankingWithPlayers[] = (data || []).map((item: any) => ({
				...
				soci_id: item.soci_id,
				mitjana_referencia: item.mitjana_referencia,
				data_entrada: item.data_entrada,
				data_actualitzacio: item.data_actualitzacio,
				player: {
			...
			if (includeActiveChallenges && rankingData.length > 0) {
				...
				rankingData = rankingData.map(ranking => ({
					...ranking,
					activeChallenges: challengesByPlayer.get(ranking.soci_id) || []
				}));
			}
	 ...
	 */
	async getActiveChallengesByPlayers(playerIds: number[]): Promise<Challenge[]> {
		const startTime = Date.now();
