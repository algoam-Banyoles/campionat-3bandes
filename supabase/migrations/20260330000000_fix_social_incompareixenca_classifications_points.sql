-- Fix: get_social_league_classifications must use punts_jugador1/2 to decide win/draw/loss
-- Rationale: incompareixença sets caramboles to 0/0 for both players, so comparing caramboles makes a "draw"
-- and both players end up with 1 point. Instead, we must compare per-player points (punts_jugador1/2).
...
BEGIN
  ...
  WITH player_matches_raw AS (
    SELECT
      ...
      -- Prefer per-player saved points; fallback to caramboles if punts_jugador* are missing (older data)
      CASE
