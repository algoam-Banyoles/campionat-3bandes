-- Correcció del mapping de camps històrics:
-- A la taula `classificacions` (esquema vell):
--   - caramboles_favor  = caramboles totals del jugador ✅
--   - caramboles_contra = entrades totals (nom misleading!)
--   - mitjana_particular = MILLOR mitjana (d'una partida), NO la general
-- Per tant:
--   - entrades_totals = c.caramboles_contra
--   - mitjana_general = caramboles_favor / caramboles_contra
--   - millor_mitjana = c.mitjana_particular
--
-- La branca "temps real" (IF v_has_inscriptions) es preserva sense canvis.
...
BEGIN
  ...
  ELSE
    -- Mapping correcte de l'esquema històric:
    --   caramboles_contra → entrades_totals
    --   mitjana_particular → millor_mitjana
    --   mitjana_general = caramboles_favor / caramboles_contra
    RETURN QUERY
    SELECT
      c.categoria_id,
      ...
      c.caramboles_contra::INTEGER as entrades_totals,
      CASE
        WHEN c.caramboles_contra > 0 AND c.caramboles_favor IS NOT NULL
        THEN ROUND(c.caramboles_favor::NUMERIC / c.caramboles_contra::NUMERIC, 3)
        ELSE NULL
      END as mitjana_general,
      c.mitjana_particular as millor_mitjana,
