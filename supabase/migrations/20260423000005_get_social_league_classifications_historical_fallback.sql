-- Afegeix fallback a la taula `classificacions` per events històrics sense inscripcions.
-- La primera branca (càlcul en temps real) preserva exactament la lògica actual
-- (post-Fase 5c: usa jugador1_soci_numero i jugador2_soci_numero, NO players).
-- La branca ELSE llegeix de la taula `classificacions` guardada, amb JOIN a socis
-- i categories, per a events previs a 2025-2026 que només tenen dades guardades.
...
BEGIN
  ...
  ELSE
    ...
    SELECT
      ...
      NULL::INTEGER as entrades_totals,
