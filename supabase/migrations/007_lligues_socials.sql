...
-- Basat en l'esquema real de producció amb sistema dual players/socis
...
-- 3. Inscripcions (mantenir compatibilitat amb players)
...
CREATE TABLE IF NOT EXISTS configuracio_calendari (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    event_id uuid NOT NULL REFERENCES events(id) ON DELETE CASCADE,
    dies_setmana text[] DEFAULT '["dl","dt","dc","dj","dv"]',
    hores_disponibles text[] DEFAULT '["18:00","19:00"]',
